alter table public.invitation_plan_pairs
  add column suggested_public_place_id uuid
    references public.public_places(public_place_id);

create index invitation_plan_pairs_suggested_public_place_id_idx
  on public.invitation_plan_pairs(suggested_public_place_id)
  where suggested_public_place_id is not null;

comment on column public.invitation_plan_pairs.suggested_public_place_id is
  'One deterministic public-place suggestion for this exact Fixed Plan pair; NULL when no eligible place exists.';

create function public.recommend_walking_public_place(
...
as $$
  ...
  eligible_places as (
    ...
    where place.category = 'park'
      and place.location_point is not null
      and place.attributes #>> '{walking_place,large_park_candidate}' = 'true'
      and jsonb_typeof(
        place.attributes #> '{walking_place,area_square_meters}'
      ) = 'number'
      and (
        place.attributes #>> '{walking_place,area_square_meters}'
...
revoke execute on function public.recommend_walking_public_place(
  double precision,
  double precision,
  double precision,
  double precision
) from public, anon;
grant execute on function public.recommend_walking_public_place(
  double precision,
  double precision,
  double precision,
  double precision
...
create function public.get_fixed_plan_invitation_suggested_place(
...
as $$
  select
    ...
    sender_plan.place_name as sender_area_name,
    receiver_plan.place_name as receiver_area_name,
...
revoke execute on function public.get_fixed_plan_invitation_suggested_place(uuid)
  from public, anon;
grant execute on function public.get_fixed_plan_invitation_suggested_place(uuid)
  to authenticated;
