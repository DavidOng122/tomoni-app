-- Create partial unique index for pending fixed plan invitations
create unique index if not exists idx_invitations_unique_fixed_plan_pending 
on public.invitations (fixed_plan_id, sender_user_id, receiver_user_id) 
where fixed_plan_id is not null and invitation_status = 'pending';

-- Create RPC for sending a fixed schedule invitation
...
begin
  ...
  if not exists (
    ...
    where m.fixed_plan_id = p_fixed_plan_id
      ...
      -- Distance <= 3km
      and public.extensions.st_distance(
        public.extensions.st_setsrid(public.extensions.st_makepoint(m.longitude, m.latitude), 4326)::public.extensions.geography,
        public.extensions.st_setsrid(public.extensions.st_makepoint(c.longitude, c.latitude), 4326)::public.extensions.geography
      ) / 1000.0 <= 3.0
      -- Time difference <= 60m
      and least(
