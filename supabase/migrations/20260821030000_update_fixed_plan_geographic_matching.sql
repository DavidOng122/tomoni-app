-- Fixed Plan同行 matching is based on activity, overlapping days, time, and
-- geographic distance. Google Place identity and display labels are not match keys.
  ...
  candidate_active_plans as (
    ...
    where fp.user_id <> v_user_id
      ...
      and not exists (
        ...
        where invitation.invitation_status in ('pending', 'accepted')
          and (
...
-- Invitation eligibility uses the same hard filters as Discover so every shown
-- candidate can be invited for the selected plan.
...
begin
  ...
  select invitation.invitation_id, conversation.conversation_id
  ...
  if not exists (
    ...
    where my_plan.fixed_plan_id = p_fixed_plan_id
      and my_plan.user_id = v_sender_id
      and my_plan.plan_status = 'active'
      and candidate_plan.user_id = p_receiver_id
      ...
      and not exists (
        ...
        where connection.connection_status = 'active'
          and (
            (connection.user_a_id = v_sender_id and connection.user_b_id = candidate_plan.user_id)
            or (connection.user_b_id = v_sender_id and connection.user_a_id = candidate_plan.user_id)
          )
      )
  ) then
