-- Event destinations may be anywhere in Edogawa. Distance remains a ranking signal,
-- but it must not remove otherwise eligible official events or cultural facilities.
...
begin
  ...
  with plan_context as (
    select
      ...
      extensions.st_setsrid(
        extensions.st_makepoint(sender_plan.longitude, sender_plan.latitude), 4326
      )::extensions.geography as sender_point,
      extensions.st_setsrid(
        extensions.st_makepoint(receiver_plan.longitude, receiver_plan.latitude), 4326
  ...
  scored_events as (
    select
      ...
      least(
        abs(extract(epoch from ((event.start_at at time zone 'Asia/Tokyo')::time - context.sender_time)) / 60.0),
        1440 - abs(extract(epoch from ((event.start_at at time zone 'Asia/Tokyo')::time - context.sender_time)) / 60.0)
      ) as sender_time_delta_minutes,
      least(
        abs(extract(epoch from ((event.start_at at time zone 'Asia/Tokyo')::time - context.receiver_time)) / 60.0),
        1440 - abs(extract(epoch from ((event.start_at at time zone 'Asia/Tokyo')::time - context.receiver_time)) / 60.0)
