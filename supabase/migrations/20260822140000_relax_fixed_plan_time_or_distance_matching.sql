-- Fixed Plan companion matching still requires the same activity, an overlapping
-- weekday, and active users/plans. A pair is now eligible when either the time
-- difference is within 90 minutes OR the straight-line distance is within 3 km.
-- Destination recommendations remain stricter and are intentionally unchanged.
...
begin
  ...
  filtered_matches as (
    select *
    from matches
    where (distance_km <= 3.0 or time_diff_minutes <= 90)
