create function public.recommend_study_reading_public_place(
...
as $$
  ...
  eligible_places as (
    ...
    where place.category = 'library'
      and place.source_dataset_id = 'edogawa_public_libraries'
...
revoke execute on function public.recommend_study_reading_public_place(
  double precision,
  double precision,
  double precision,
  double precision
) from public, anon;
grant execute on function public.recommend_study_reading_public_place(
  double precision,
  double precision,
  double precision,
  double precision
...
with study_reading_recommendations as (
  select
    pair.invitation_id,
  ...
  join public.fixed_plans sender_plan
    ...
    and sender_plan.activity_type = 'study_reading'
  join public.fixed_plans receiver_plan
    on receiver_plan.fixed_plan_id = pair.receiver_fixed_plan_id
    and receiver_plan.activity_type = 'study_reading'
  cross join lateral public.recommend_study_reading_public_place(
    sender_plan.latitude,
    sender_plan.longitude,
    receiver_plan.latitude,
    receiver_plan.longitude
...
from study_reading_recommendations recommendation
