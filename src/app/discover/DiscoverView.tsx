...
import { ACTIVITY_ICONS } from '@/features/fixed-schedules/lib/constants';
        ...
        <div className={styles.topActions}>
          ...
          <button
            ...
            aria-label={pendingNotificationCount > 0
              ? `通知：未対応のお知らせが${pendingNotificationCount}件あります`
              : '通知'}
          >
          ...
          {currentActivity && (
            <section aria-label="現在の活動">
              <Link
                ...
                href={`/connections/plans/${currentActivity.conversationId}`}
                aria-label={`${currentActivity.name}さんとの同行詳細を見る`}
              >
                  ...
                  <div className={styles.recommendationPanel} key={group.fixedPlanId}>
                    ...
                    >
                      <span className={styles.activityIcon} aria-hidden="true">
                        <Image
                          src={ACTIVITY_ICONS[group.activityType]}
                          width={18}
                          height={18}
                          alt=""
                        />
                      ...
                      <div>
                        <strong>{getRecommendationHeading(group.title)}</strong>
                        ...
                        {group.recommendations.map((rec) => (
                          ...
                          >
                            <div className={styles.personLine}>
                              ...
                              <span>{getRecommendationNickname(rec.profile.nickname)}</span>
                            </div>
                            <div className={styles.tagList}>
                              {rec.match.reasons.slice(0, 2).map((reasonCode) => (
                                <span key={reasonCode} style={getTagStyle(reasonCode)}>
                                  {getRecommendationReasonLabel(reasonCode, group.title)}
                                </span>
                              ))}
                            </div>
                          </div>
