import { EDOGAWA_RECREATION_DESTINATION_HEADERS } from "./edogawaPublicPlaceHeaders.ts";
import type { EdogawaMapPlace, ParsedEdogawaMapSource } from "./edogawaPublicPlaceSourceTypes.ts";
...
export function parseEdogawaRecreationDestinationsCsv(
  bytes: Uint8Array,
  ...
  const parsed = parseCsvRecords(decodeCsvBytes(bytes, encoding), {
    ...
    knownHeaders: EDOGAWA_RECREATION_DESTINATION_HEADERS,
  });
  ...
  return {
    ...
    places: parsed.rows.map((row, index): EdogawaMapPlace => ({
      ...
      name: value(row["名称"]) ?? "",
      description: value(row["説明"]) ?? value(row["コメント"]),
