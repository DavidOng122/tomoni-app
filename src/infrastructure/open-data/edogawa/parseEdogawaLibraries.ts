import {
  EDOGAWA_LIBRARY_MAP_HEADERS,
  EDOGAWA_LIBRARY_STANDARD_HEADERS,
...
export function parseEdogawaLibraryStandardCsv(bytes: Uint8Array): ParsedEdogawaStandardSource {
  const parsed = parseCsvRecords(decodeCsvBytes(bytes, "utf-8"), {
    requiredHeaders: ["ID", "名称", "所在地_連結表記", "利用可能曜日", "開始時間", "終了時間"],
    knownHeaders: EDOGAWA_LIBRARY_STANDARD_HEADERS,
  });
  return {
    headers: parsed.headers,
    unknownHeaders: parsed.unknownHeaders,
